...
pro sj50,koppa,clusterF,TruthF,path,koppaFile,newLog,locate;,explore
  COMPILE_OPT IDL2
  ...
  ;cluster聚类类别
  clusterNUMs=numsClass(cluster)
  explore=valueJudge(cluster,maskClass,koppaFile,coeffient,valuable)
  koppa=kappaCal(coeffient,valuable,classNUMs,clusterNUMs,explore,locate)

  ;得到聚类的炸开
  ok=blastCluster2(explore,cfid,fid,newLog)
...
; path --> fid
;-
function blastCluster2,explore,cfid,fid,newLog
  compile_opt idl2
...
;+
; :Description:
;从属度 same/older;价值度  same/itself
;-
function valueJudge,cluster,maskClass,koppaFile,coeffient,valuable
  compile_opt idl2
  ...
  foreach clus,clusterNUMs do begin
    ...
    foreach cls,classNUMs  do begin
      ...
      new=new eq 2
      countNew=total(new);count
      countCls=total(maskData);count
      new=[]
      maskData=[]
  ...
  explore=exploreAll(clusterNUMs,classNUMs,coeffient,valuable)
...
;为每一个聚类的类注释需要分解成几个
;-
function exploreAll,clusterNUMs,classNUMs,coeffient,valuable
  compile_opt idl2
...
;+
; :Description:
; blast certain class follow direct of explore
; as well as obtain the
;-
function kappaCal,coeffient,valuable,classNUMs,clusterNUMs,explore,locate
  compile_opt idl2
  ...
  foreach ex,explore do begin;line
    ...
    if n_ex ge 1 then begin
      ...
      if valuable[index[0],e_Index] gt 0.1 then locate[e_Index]=index
    endif
    e_Index++
  endforeach
